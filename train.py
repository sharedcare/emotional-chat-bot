...
from utils import *
from data_loader import load_data_flatten, get_batch_data_flatten
...
def main(**kwargs):
    ...
    if kwargs['model'] == 'Seq2Seq':
        net = Seq2Seq(
            len(src_w2idx), kwargs['embed_size'], len(tgt_w2idx),
            kwargs['utter_hidden'],
    ...
    for epoch in pbar:
        ...
        if kwargs['hierarchical'] == 1:
            raise Exception("Currently not supported hierarchical models")
        else:
            train_iter = get_batch_data_flatten(
                kwargs['src_train'],
                kwargs['tgt_train'],
                kwargs['src_vocab'],
                kwargs['tgt_vocab'],
                ...
                kwargs['maxlen'],
                kwargs['tgt_maxlen'])
            test_iter = get_batch_data_flatten(
                kwargs['src_test'],
                kwargs['tgt_test'],
                kwargs['src_vocab'],
                kwargs['tgt_vocab'],
                ...
                kwargs['maxlen'],
                kwargs['tgt_maxlen'])
            dev_iter = get_batch_data_flatten(
                kwargs['src_dev'],
                kwargs['tgt_dev'],
                kwargs['src_vocab'],
                kwargs['tgt_vocab'],
        ...
        if epoch > int(kwargs['epochs'] * 0.8):
            ...
            state = {
                'net': net.state_dict(), 'opt': optim_state,
                'epoch': epoch, 'patience': patience
                }
            torch.save(state, f'./ckpt/{kwargs["dataset"]}/{kwargs["model"]}/vloss_{val_loss}_epoch_{epoch}.pt')
